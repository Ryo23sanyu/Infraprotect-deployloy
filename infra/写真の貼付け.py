...
step = 14
num_positions = len(data) * 3  # データ数に3列分を掛けています
...
for item in data: # リスト(data)をforループの要素(item)に代入
...
for part_pos, number_pos, name_pos, lank_pos, memo_pos in zip(partsname_cell_positions, number_cell_positions, damagename_cell_positions, lank_cell_positions, damage_memo_cell_positions):
    # 3つの列ごとにデータを入力します
    for idx, (part_cell, number_cell, name_cell, lank_cell, memo_cell) in enumerate(zip(part_pos, number_pos, name_pos, lank_pos, memo_pos)):
        print(f"index: {idx}")
        if data_index < len(data) and data[data_index]['last'] is not None:  # データが残っている場合は入力
            picture_path = data[data_index]['picture']
            #print(f"Checking picture: '{picture_path}' (index: {data_index})")  # デバッグ出力
            if idx == 2 and picture_path is not None:
                #print(f"Skipping index {data_index} due to picture: '{picture_path}'")  # デバッグ出力
                continue
            # firstキーの内容を左側のセルに入力
            first_data = data[data_index].get('first', [['']])[0][0] # 排水管 Dp0101
            split_space = first_data.split(" ")
            first_part_data = split_space[0]

            match = re.search(r'\d+', split_space[1])
            first_number_data = match.group()

            ws[part_cell] = first_part_data # 排水管
            ws[number_cell] = first_number_data # 0101
            print(f"first: {first_part_data}__{first_number_data}")
            # secondキーの内容を右側のセルに入力
            second_data = data[data_index].get('second', [['']])[0][0] # ①腐食(大大)-e
            
            # 最初の文字を取り出して辞書に基づいて置換
            second_damage_name = second_data[0] # 先頭の文字を抽出「①」
            second_name_data = number_change.get(second_damage_name, second_damage_name)

            second_lank_data = second_data[-1]

            ws[name_cell] = second_name_data # ①を辞書変換
            ws[lank_cell] = second_lank_data # e
            print(f"second: {second_name_data}__{second_lank_data}")
            # secondキーの内容を右側のセルに入力
            memo_data = data[data_index].get('textarea_content', [['']]) # 排水管に拡がりのある腐食,点錆が見られる。\n【関連損傷】\n排水管 Dp0102:⑤防食機能の劣化(分類1)-e
