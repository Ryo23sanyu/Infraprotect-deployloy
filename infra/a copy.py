        ...
        if entity.dxftype() == 'MTEXT':
            ...
            if len(cad_data) > 0 and not text.startswith("※") and not any(keyword in text for keyword in ["×", ".", "損傷図"]):
             # 改行を含むかどうかをチェックする(and "\n" in cad):# 特定の文字列で始まるかどうかをチェックする: # 特定の文字を含むかどうかをチェックする
                ...
                related_text = "" # 見つけたMTextと関連する文字列を代入する変数
                # MTextの下、もしくは右に特定のプロパティ(Defpoints)で描かれた文字を探す
                for neighbor in msp.query('MTEXT[layer=="Defpoints"]'):
                ...
                if  len(related_text) > 0: #related_textが見つかった場合
                    cad_data.append(related_text) # related_textを追加する
...
def entity_extension(mtext, neighbor):
    ...
    y_start = mtext_insertion[1]# + mtext.dxf.char_height * 1 # Y開始位置
    # Y終了位置 下部なのでY開始位置-文字の高さ＊(行数+1)
    y_end  = mtext_insertion[1] - mtext.dxf.char_height *( text_lines_count + 1 )
